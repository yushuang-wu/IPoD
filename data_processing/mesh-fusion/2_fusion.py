...
class Fusion:
    ...
    def get_parser(self):
        ...
        parser.add_argument('--resolution', type=float, default=512, help='Resolution for fusion.')
    ...
    def run_fuse(self):
        ...
        for filepath in files:
            ...
            if filepath.endswith('.off'):
                libmcubes.export_off(vertices, triangles[:, ::-1], off_file)
            elif filepath.endswith('.obj'):
                libmcubes.export_obj(vertices, triangles[:, ::-1], off_file)
            elif filepath.endswith('.ply'):
                # libmcubes.export_ply(vertices, triangles[:, ::-1], off_file)
                mesh_new = trimesh.Trimesh(vertices=vertices, faces=triangles[:, ::-1])
                mesh_new.export(off_file, file_type='ply')
            else:
                # libmcubes.export_ply(vertices, triangles[:, ::-1], off_file)
                mesh_new = trimesh.Trimesh(vertices=vertices, faces=triangles[:, ::-1])
                mesh_new.export(off_file)
